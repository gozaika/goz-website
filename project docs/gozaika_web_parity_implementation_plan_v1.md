...
| Web W5 | Partner surface recomposition + switcher | W3 | **In progress 2026-06-29** — 6/10 portal surfaces done: dashboard, orders, drops(+new), finance, reports, templates, all demo-OWNER browser-verified. Partner label reconciled **"Zayka Pro" → "goZaika Partner"** (owner-approved §9.4) across dashboard + login + portal-nav + onboarding. MetricHero gained additive `titleAs` (h1 semantics). Autonomous decision log: `docs/web/w5-w7-autonomous-decisions.md`. **Remaining:** onboarding, compliance, profile, reviews + RestaurantSwitcher in chrome. |
