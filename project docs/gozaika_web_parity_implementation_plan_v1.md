...
| Web W5 | Partner surface recomposition + switcher | W3 | **In progress 2026-06-29** — 1/10 portal surfaces done: `/portal/dashboard` recomposed (MetricHero/DataTable/Sparkline/SellThroughBar/ActionCard), demo-OWNER browser-verified. Partner label reconciled **"Zayka Pro" → "goZaika Partner"** (owner-approved §9.4, matches mobile twin) across dashboard + login + portal-nav + onboarding. MetricHero gained additive `titleAs` (h1 semantics). **Remaining:** orders, drops(+new), templates, finance, reports, onboarding, compliance, profile, reviews + RestaurantSwitcher in chrome. |
