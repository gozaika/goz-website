...
| Web W5 | Partner surface recomposition + switcher | W3 | **In progress 2026-06-29** — 3/10 portal surfaces done: `/portal/dashboard`, `/portal/orders`, `/portal/drops`(+`/new`), all demo-OWNER browser-verified. Partner label reconciled **"Zayka Pro" → "goZaika Partner"** (owner-approved §9.4) across dashboard + login + portal-nav + onboarding. MetricHero gained additive `titleAs` (h1 semantics). Autonomous decision log: `docs/web/w5-w7-autonomous-decisions.md`. **Remaining:** templates, finance, reports, onboarding, compliance, profile, reviews + RestaurantSwitcher in chrome. |
