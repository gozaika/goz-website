...
| Mobile Slice 16 | Push, deep links, native permissions and offline hardening | 7,9,12 | **Code done (2026-06-29):** expo-notifications token registration (both apps), FCM HTTP v1 server sender (service-account, env-gated; send proven 4/4), deep-link routing, offline banners. Gate 7/7. **Device delivery gated on `google-services.json` per app** (owner). |
