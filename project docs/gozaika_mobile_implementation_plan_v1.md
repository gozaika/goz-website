...
- Web visual QA is the recommended first step when resuming (run `npm.cmd --workspace @gozaika/consumer-web run dev`, open `/restaurants/[slug]` + `/drops`): confirm hero overlay contrast and banner crop. SVG paths are verified to match `cuisineCoverKey` output, but the composition hasn't had an on-screen capture yet.
