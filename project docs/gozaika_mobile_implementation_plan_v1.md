...
- **⚠ Discovered (not D1b — pre-existing cloud demo-data corruption):** restaurant `story_markdown` and drop `bag_short_description` copy render literal `???` where em-dashes belong (e.g. Sweet Bytes story “200 items a day ??? croissants … choux ??? all from scratch”; multiple drop blurbs). The **seed source is correct** — `supabase/seed_demo/demo_seed.sql:643` has proper `—`; the cloud DB stored `???`, so the em-dashes were mangled at seed-apply time (non-UTF-8 psql client on Windows). Pervasive on the exact presales surfaces D1 targets. Fix is a contained data repair (re-apply the affected seed copy / or `UPDATE … replace('???','—')`) on the cloud demo DB — out of D1b scope, tracked for a follow-up.
