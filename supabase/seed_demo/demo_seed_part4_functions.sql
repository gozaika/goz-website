...
begin
  ...
  for v_rst_idx in 1 .. array_length(v_target_rsts, 1) loop
    ...
    insert into order_order (
      ...
      'CONFIRMED', 'CAPTURED',
      encode(digest('nonce:live:' || v_order_a_pk::text, 'sha256'), 'hex'),
      encode(digest('otp:live:'   || v_order_a_pk::text, 'sha256'), 'hex'),
      v_pickup_start, v_pickup_end,
    ...
    insert into order_order (
      ...
      'CONFIRMED', 'CAPTURED',
      encode(digest('nonce:live:' || v_order_b_pk::text, 'sha256'), 'hex'),
      encode(digest('otp:live:'   || v_order_b_pk::text, 'sha256'), 'hex'),
      v_pickup_start, v_pickup_end,
      v_rev.suggested_price_paise, 0, 0, v_rev.suggested_price_paise, 'INR',
      v_rst.restaurant_name, v_rst.restaurant_slug,
      v_drop_title, v_rev.display_name,
