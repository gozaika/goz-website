-- Repair webhook conversion RPC column/return-value ambiguity.
--
-- api_convert_paid_hold_to_order returns a column named payment_order_intent_pk.
-- PL/pgSQL also sees that output column as a variable, which made statements
-- such as `where payment_order_intent_pk = ...` ambiguous. Prefer table
-- columns inside embedded SQL statements while preserving the existing return
