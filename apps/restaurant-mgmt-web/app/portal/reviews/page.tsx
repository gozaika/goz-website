...
function StarDisplay({ value }: { readonly value: number }) {
  return (
    <span className="flex items-center gap-0.5" aria-label={`${value} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <span key={n} className={`text-base ${n <= value ? "text-gold" : "text-charcoal/15"}`} aria-hidden="true">
          ★
        </span>
      ))}
    </span>
...
function CategoryBar({ label, value }: { readonly label: string; readonly value: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-32 shrink-0 text-xs text-charcoal/60 capitalize">{label.replace(/_/g, " ")}</span>
    ...
    <PortalChrome restaurantName={restaurants[0]?.restaurantName} statusCode="ACTIVE">
      <section className="px-4 py-6 sm:px-6">
        <div className="mb-6">
          ...
          <p className="mt-1.5 max-w-2xl text-sm text-charcoal/60">
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-forest/20 bg-success-soft px-8 py-6 text-center">
            {averageRating != null ? (
              <>
                ...
                <p className="mt-1 text-xs text-charcoal/55">{approved.length} approved review{approved.length !== 1 ? "s" : ""}</p>
              </>
            ) : (
              <>
                <p className="text-4xl font-black text-charcoal/25">—</p>
                <p className="mt-2 text-xs text-charcoal/50">No approved reviews yet</p>
              </>
            )}
          </div>
          ...
          <div className="rounded-xl border border-hairline bg-white p-5">
            {categoryAverages.length > 0 ? (
              <div className="grid gap-2.5">
                <p className="mb-1 text-xs font-bold uppercase tracking-wide text-charcoal/50">Category breakdown</p>
              ...
              <div className="flex h-full items-center">
                <p className="text-sm text-charcoal/50">Category breakdown will appear once reviews include ratings.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          <div className="rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm">
            ...
            <span className="ml-1 text-charcoal/55">total</span>
          </div>
          {pending.length > 0 && (
          ...
          <div className="rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm">
            ...
            <span className="ml-1 text-charcoal/55">approved &amp; public</span>
          </div>
        </div>
        ...
        {reviews.length === 0 ? (
          <div className="rounded-xl border border-hairline bg-white px-6 py-12 text-center">
            <p className="text-sm font-semibold text-charcoal/50">No reviews yet</p>
            <p className="mt-1 text-xs text-charcoal/40">Reviews from verified BAM Bag pickups will appear here after moderation.</p>
          </div>
        ) : (
            ...
            {reviews.map((review) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      {review.bag_display_name && (
                        <p className="text-xs text-charcoal/50">{review.bag_display_name}</p>
                      )}
                    </div>
                  ...
                  {review.review_text && (
                    <p className="mt-3 text-sm text-charcoal/80 leading-relaxed">{review.review_text}</p>
                  )}
                  ...
                  {review.categories && Object.keys(review.categories).length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {Object.entries(review.categories).map(([key, val]) => (
                        typeof val === "number" ? (
                          <span key={key} className="rounded-full border border-hairline bg-cream px-2.5 py-0.5 text-xs text-charcoal/60">
