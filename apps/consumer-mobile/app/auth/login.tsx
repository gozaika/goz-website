...
export default function LoginScreen() {
  ...
  const { loginState, session, consentAcknowledged, requestOtp, verifyOtp, resendOtp } = useAuth();
  ...
  useEffect(() => {
    if (session) {
      router.replace(consentAcknowledged ? "/" : "/onboarding/consent");
    }
  }, [session, consentAcknowledged, router]);
  ...
  const phoneE164 = loginState.step === "otp" ? loginState.phoneE164 : loginState.step === "verifying" ? loginState.phoneE164 : "";
