...
/**
 * Register this device's native push token (Slice 16) once the consumer is
 * signed in. Uses the FCM/APNs device token (server sends via FCM HTTP v1).
 * Fails safe: on a simulator, when permission is denied, or before
 * `google-services.json` is embedded, `getDevicePushTokenAsync` throws and we
