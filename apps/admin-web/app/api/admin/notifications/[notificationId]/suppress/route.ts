...
import {
  notificationSuppressRequestSchema,
  type ApiResponse,
...
function mapSuppressError(message: string): { readonly error: string; readonly status: number } {
  ...
  if (message.includes("already sent")) return { error: "Already sent notifications cannot be suppressed.", status: 409 };
  if (message.includes("reason")) return { error: "Add a suppression reason.", status: 400 };
  return { error: "Could not suppress notification.", status: 500 };
}
...
export async function POST(request: Request, { params }: { readonly params: Promise<{ readonly notificationId: string }> }) {
  ...
  const parsed = notificationSuppressRequestSchema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success) {
    return NextResponse.json(
      { ok: false, error: parsed.error.issues[0]?.message ?? "Add a suppression reason." } satisfies ApiResponse,
      { status: 400 },
    );
  }

  const service = createServiceRoleSupabaseClient();
  const { data, error } = await service.rpc("api_suppress_notification", {
  ...
  if (error) {
    const mapped = mapSuppressError(error.message);
  ...
  if (!row) {
    return NextResponse.json({ ok: false, error: "Notification was not suppressed." } satisfies ApiResponse, { status: 500 });
  }
