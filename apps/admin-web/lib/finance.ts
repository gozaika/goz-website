...
type SettlementSummaryRow = {
  ...
  readonly external_document_ref?: string | null;
...
type EligiblePreviewRow = {
  ...
  readonly refund_paise: number | string;
  readonly commission_bps: number | string;
  readonly commission_plan_code: string | null;
  readonly commission_paise: number | string;
  readonly net_payout_paise: number | string;
  readonly eligibility_status_code: FinanceEligibleOrderPreview["eligibilityStatusCode"];
  readonly exclusion_reason_code: string | null;
  readonly exclusion_reason_text: string | null;
};
...
export function mapFinanceSettlementSummary(row: SettlementSummaryRow): FinanceSettlementSummary {
  return {
    ...
    invoice: {
      ...
      externalDocumentRef: row.external_document_ref ?? null,
    },
...
export function mapEligiblePreview(row: EligiblePreviewRow): FinanceEligibleOrderPreview {
  return {
    ...
    exclusionReasonText: row.exclusion_reason_text,
  };
}
