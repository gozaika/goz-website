...
- `docs/mobile/specs/mobile-shared-architecture-and-release-spec-v1.md`
...
| Mobile Slice 12 | Restaurant onboarding, compliance and profile | 4,6 | **Done (2026-06-28):** profile/basics + compliance document upload (earlier) + **resumable onboarding wizard + location pin** (this push). Gate 7/7; smoke `scripts/smoke/slice12-onboarding-smoke.mjs`; on-device proof = `expo-location` Android build (batched per Plan B). |
...
| Mobile Slice 16 | Push, deep links, native permissions and offline hardening | 7,9,12 | **Done + device-proven (2026-06-29):** expo-notifications token registration (both apps), FCM HTTP v1 server sender (service-account, env-gated), deep-link routing, offline banners. Gate 7/7; send smoke 4/4; **live push delivered to the Pixel 7a** (google-services.json wired per app). Biometric app-lock / App Links association = deferred follow-ups. |
| Mobile Slice 17 | Accessibility, security, observability and performance gate | 8–16 | **Done (code-side, 2026-06-29):** product-media gate #5 locked by tests (`resolveProductMedia` + `resolveDropImage` trust boundary) + live smoke 5/5 (0 ingest leaks); push deep-link injection guard (`safeInternalPath`, both apps); release bundle secret scan (clean); a11y input labels across all forms (on top of X1 contrast/reduced-motion/Dynamic-Type); redacting logger + SecureStore confirmed; bundle sizes measured (consumer 4.65 MB / partner 4.71 MB Hermes). Parity ledger reconciled. Gate 7/7. Audit: `docs/mobile/slice17-hardening-audit.md`. **Human a11y + security sign-off + on-device sweep batched.** |
| Mobile Slice 18 | Store packages, beta and staged production release | 17 | **Done (code/config-side, 2026-06-29):** EAS gaps reconciled — G1 projectId (both), G2 `runtimeVersion` policy (added both), G6 push (S16), G7 prod env pinned all **closed**; G3 submit creds / G4 versionName / G5 crash-SDK remain **owner-gated**. Partner permission minimization: blocked unused `RECORD_AUDIO` (camera/location only) + reproducible `scripts/store-launch/permissions-manifest.mjs`. Release-readiness doc `docs/mobile/slice18-release-readiness.md` (Data-Safety answers, staged 5→25→100 rollout + halt/rollback, reviewer/account-deletion status, **owner-authorization action list**). Gate 7/7. **External submissions/signing/rollout require owner authorization at action time.** |
...
| D1 | Demo/presales readiness | U1-R4, X1 | Complete (D1a–D1f; 14 store cards in docs/archived/launch-assets-pre-factory/legacy-store-assets/store-assets-v1/) 2026-06-27; owner to inspect + FF to `main` |
...
- **Sub-slices:** **D1a** mobile cuisine cover art (done); **D1b** web (consumer-web + restaurant-web) cuisine art wiring (SVG native); **D1c** drop-type/blind-bag art variants + cuisine-accurate mapping if the DTO exposes a cuisine code; **D1d** store-asset cards composed from the now-vibrant states (reconcile with the `.codex-artifacts/archive/launch-assets-pre-factory/gozaika-polish-v2` track — single-agent ownership); **D1e** Maestro demo-flow polish + capture manifest.
...
- **D1b done — web cuisine art + shared resolver:** extracted the cuisine-keyword logic into `@gozaika/utils#cuisineCoverKey(name, cuisineTags?)` (+ `COVER_KEYS`, `CoverKey`) as the single source of truth; refactored mobile `mediaFallbacks.coverFor` to use it. Copied the 5 cover SVGs to `apps/consumer-web/public/art/`. Wired web: the restaurant-detail hero (`restaurants/[slug]/restaurant-detail-client.tsx`) now layers the cuisine art behind a forest `/92–/96` gradient overlay (white text stays AA-readable); the shared `@gozaika/ui` `DropCard` shows a cuisine cover banner (so every consumer-web list drop card gets art); the inline "Closing soon" cards in `drops/drop-discovery-client.tsx` match. Only consumer-web renders the shared `DropCard` (admin/restaurant-web reference only the `PublicDropCard` type), so no other app needs `/art/`. Typecheck: `@gozaika/ui`, `@gozaika/consumer-web`, `@gozaika/utils`, consumer-mobile all pass; mobile gate 7/7.
- **D1b web visual QA done (2026-06-27):** captured on-screen against the running `consumer-web` dev server with **live cloud demo data** (via the preview harness + a one-off Playwright capture at viewport 1280×900). Verified: (1) restaurant-detail **hero overlay keeps text AA-readable** — white headline + gold neighborhood/“See reviews” stay crisp over the forest `/92–/96` gradient on all three cuisine extremes (biryani `biryani-baithak`, lightest cover `sweet-bytes-bakery`, grill `the-smoky-grill`); the cuisine art shows through only subtly; (2) the shared `DropCard` **cover banner is cuisine-accurate and crops cleanly** — full-bleed (`-mx-4 -mt-4 … object-cover`) and clipped to the card’s `overflow-hidden rounded-lg`, with `cuisineCoverKey` resolving correctly across the live grid (Andhra Spice Trail → coastal fish, Sattvik Kitchen → thali, The Smoky Grill → grill); (3) the “Closing soon” cards use the identical crop technique (no live active drops to render — cloud demo windows have passed — but same CSS verified). No art-wiring fixes needed. Evidence (gitignored, on disk): `.codex-artifacts/mobile-ux-uplift/web-preview/d1b-hero-{biryani,bakery,grill}.png` + `d1b-drops-cards.png`.
- **D1 demo-data mojibake repair done (2026-06-27, owner-directed):** discovered during D1b QA — restaurant `headline`/`story_markdown`, `catalog_bag_template_revision.short_description`/`included_item_hint_text`, and `drop_drop.drop_title` rendered literal `???` where em-dashes belong (e.g. Sweet Bytes “200 items a day ??? croissants … choux ??? all from scratch”; every drop title). The **seed source is correct UTF-8** (`demo_seed.sql:643` has `—`); the cloud DB mangled em-dashes (U+2014, 3-byte UTF-8) to `???` at seed-apply time (non-UTF-8 psql client). Audited every non-ASCII char in these columns' demo copy — **all em-dashes**, no en-dashes/ellipses/accents — and verified each `??? → —` correction matches the seed verbatim (27/27 rows). Applied to the cloud demo DB via service-role PATCH per row (27 rows); re-survey across 11 public tables = **0 remaining `??`-runs**. Added the idempotent, guarded repair `supabase/seed_demo/demo_fix_mojibake.sql` (re-run after any future seed apply if `???` reappears). Pure data fix — no source/schema/app change; mobile gate stays 7/7. Fixed-state evidence (gitignored): `.codex-artifacts/mobile-ux-uplift/web-preview/d1fix-*.png`.
- **D1c done — drop-type / blind-bag art variants:** shared resolver in `@gozaika/utils`: `dropCoverKey(drop)` returns the cuisine-agnostic `"mystery"` cover for `BLIND_ADVENTURE` (so blind bags no longer reveal their cuisine) and the cuisine cover otherwise; `dropTypeRibbon(code)` returns "Chef's Special" / "Spotlight" for the premium types (null for standard/blind). New original art `cover-mystery` (forest cloche + gold "?" + steam) added to `scripts/demo-art/build-art.mjs` (SVG → mobile PNG + copied to `consumer-web/public/art/`). Web: the shared `@gozaika/ui` `DropCard` and the consumer-web "Closing soon" cards now use `dropCoverKey` for the banner and overlay a gold ribbon (charcoal-on-gold, AA) for premium types. Mobile: `mediaFallbacks.coverForDrop(drop)` picks the mystery PNG for blind bags; wired into the 3 drop fallbacks (DropCard, Home rail, drop detail). Restaurant covers stay cuisine-based (`coverFor`). 6 new utils tests; typecheck (utils/ui/consumer-web/consumer-mobile) + mobile gate 7/7. **Web QA captured (2026-06-27):** all 4 drop types confirmed on `/drops` against live cloud data — `cover-mystery` on the Sweet Bytes blind bag (no longer the bakery cover), ★ Chef's Special (Bawarchi/Andhra) + ★ Spotlight (Smoky Grill) ribbons rendering with AA contrast; evidence `.codex-artifacts/mobile-ux-uplift/web-preview/d1c-*.png`. Mobile is wired + bundles (expo export passes); on-device Android capture available via `scripts/android-preview-install.ps1` (deferred to the slice-D manual inspection).
- **D1d done — store-asset reconciliation (not cards):** reconciled with the authoritative store-asset track `.codex-artifacts/archive/launch-assets-pre-factory/gozaika-polish-v2/`. Its locked decisions forbid producing store finals outside its production slices ("no final images/videos before the relevant production slices"; sources must be approved/clean; Slice 6 customer cards are blocked), and the store-launch **lane split** assigns polished creatives to **Codex**, raw captures/vibrant states to the **source-code agent**. So building competing "store-listing cards" here would duplicate/conflict + overstep the lane. D1d instead delivers `docs/mobile/d1d-store-asset-reconciliation.md`: records that D1a/D1c art + the mojibake repair materially improved the polish-v2 **customer** source material (CCard 1/2/3/7 — cuisine covers, drop-type ribbons, the blind-bag mystery cover correctness fix, clean em-dash copy), with **partner** sources unaffected (D1 art is consumer-only); and hands off the exact native recapture requirements (roll demo windows forward via `demo_prepare.sql`, recapture home/drops-list/detail from a release build). Did **not** write into the polish-v2 tree (avoids cross-track conflict). No source/gate impact (docs-only); web evidence already on disk.
- **D1e done — Maestro demo-flow polish + capture manifest:** added `docs/mobile/d1e-demo-capture-manifest.md`, a repo-level demo/presales capture index — every `apps/*/.maestro/` demo flow with its persona (LOCAL `test_otp`), preconditions, emitted screenshots, and the slice-D vibrant states it shows — cross-referencing (not duplicating) the `marketing-videos`, `polish-v2`, and `store-launch` packages. Demo-flow polish: new additive, title-independent, sign-in-free `apps/consumer-mobile/.maestro/demo-discovery-vibrant.yaml` that captures Home rail + Drops grid (showcasing D1a cuisine covers + D1c mystery/ribbon variants where active); anchors only on "Browse drops" + scroll so it stays green as the seed changes; captures to `.codex-artifacts/mobile-ux-uplift/demo-captures/` (gitignored). Not executed here (no device/emulator this session) — run it in the slice-D device inspection. Mobile gate 7/7 (the Maestro secret-scan covers the new flow).
- **D1e run on device:** `demo-discovery-vibrant.yaml` executed green on the Pixel 7a (3 stills to `.codex-artifacts/mobile-ux-uplift/demo-captures/`); on a release build, foreground the app first (devclient-safe flow has no `launchApp`).
- **D1f done — store cards PRODUCED (owner-directed; Codex stopped, this agent owns polish-v2 now):** since the lane split no longer applies, D1d's deferral is superseded — D1f produces the actual cards. Real native Pixel 7a captures of the live cloud demo (windows rolled forward via the sanctioned `demo_prepare_for_demo(cleanup,create=false)`; consumer rebuilt via `android-preview-install.ps1` so D1a cuisine art + D1c blind-bag mystery cover ship natively) → **7 customer (C1–C7) + 7 partner (P1–P7)** store cards. Pipeline `docs/archived/launch-assets-pre-factory/legacy-scripts/store-cards/build-cards.mjs` (+ `cards.config.mjs`) renders 1080×1920 Android masters via Playwright to the polish-v2 design-system lock (palette, type scale, safe zones, device frames, allowed badges, real-UI-only). Partner captured signed-in as **OWNER** (Bawarchi), unblocking polish-v2's previously-blocked P4/P5/P6. Deliverables in the **tracked** `docs/archived/launch-assets-pre-factory/legacy-store-assets/store-assets-v1/` (the polish-v2 tree is gitignored) so they push to `origin`. Caveats in `docs/archived/launch-assets-pre-factory/legacy-store-assets/store-assets-v1/README.md`: demo `GZ-SEED` IDs, partner revenue/AOV needs product/compliance approval (overlays make no financial claim), Zayka/goZaika passport naming, iOS 1290×2796 derivative, final typography. C5 pickup uses the real SMS-pickup-code UI (no fabricated QR/OTP).
- **D1 complete (D1a–D1f), 2026-06-27.** Imagery + demo-readiness + store-cards slice (the seed was already data-rich): D1a mobile cuisine covers, D1b web cuisine art + shared resolver, D1c drop-type/blind-bag variants, owner-directed cloud mojibake repair, D1d store-asset reconciliation, D1e demo-flow polish + manifest, D1f produced 14 store cards. Branch `codex/mobile-ux-uplift/d1-demo-presales`; gate 7/7; pushed to `origin`. Owner to manually inspect, then fast-forward to `main`.
...
**Status — ✅ Slice 12 COMPLETE (2026-06-28).** Resumable onboarding wizard + location pin landed this push. **Location pin:** `latitude`/`longitude` added to `restaurantProfileDataSchema` (+ DTO) and `loadRestaurantProfile` (joins `geo_address` via `restaurant_restaurant.geo_address_fk`); new canonical `restaurantLocationUpdateSchema` + BFF `PATCH /api/mobile/v1/restaurant/location` (`manageProfile`, scoped) writing coords to a created/linked `geo_address` row (`lib/mobile/location.ts`); mobile `useUpdateRestaurantLocation` + a `LocationCard` on `profile.tsx` with manual entry **and** `expo-location` foreground GPS ("Use my current location"; permission-denied → manual). **Resumable wizard:** new `restaurantOnboardingDataSchema` (derived steps + tasks, `+fixture/test`); BFF `GET /api/mobile/v1/restaurant/onboarding` (`viewDashboard`) derives 6 step `done` flags from **real** rows (basics, location pin, pickup instructions, ≥1 document, ≥1 template, ≥1 drop) + returns `restaurant_onboarding_task` rows; `PATCH` (`manageProfile`) transitions a manual-ack task (server-authoritative → resumes across sessions); `lib/mobile/onboarding.ts`; mobile `src/api/onboarding.ts` + `app/onboarding.tsx` replaces the Placeholder (progress bar + step deep-links + per-task Start/Mark-done/Reopen). `expo-location ~55.1.11` added + app.json plugin/permissions. Gate 7/7; smoke `scripts/smoke/slice12-onboarding-smoke.mjs` (onboarding 200 + 6 steps; profile lat/long; PATCH location sets→LOCATION step done; task→COMPLETED; bogus→404; clear; PICKUP_STAFF→403; unauth→401). On-device GPS proof = `expo-location` Android build, batched per Plan B (`docs/mobile/deploy-verification-checklist.md`). Earlier compliance-upload record below stays valid.
...
**Status — Code DONE 2026-06-29 (device delivery owner-gated on `google-services.json`).** Provider path is **FCM HTTP v1** via the Firebase **service-account** (server secret, env-only: `FCM_SERVICE_ACCOUNT_JSON` | `FCM_SERVICE_ACCOUNT_PATH`; gitignored — never committed). No new migration needed: the existing `notification_device` / `notification_outbox` / `notification_delivery_attempt` tables cover token lifecycle + delivery. **Built:** `expo-notifications ~55.0.24` (both apps + plugin); contract `packages/types/src/mobile/notifications.ts` (+test); BFF `POST/DELETE /api/mobile/v1/notifications/device` on both BFFs (`withMobileAuth`, per-profile; upsert reassigns on reinstall, DELETE deactivates); client `src/push/push.ts` (native FCM/APNs token via `getDevicePushTokenAsync`, fails safe pre-google-services.json) + `PushRegistrar` (session-gated) in both root layouts; sender `consumer-web/lib/push/fcm.ts` (RS256 JWT → OAuth → FCM v1; `sendPushToProfile` records outbox+attempt, deactivates unregistered tokens) + `POST /notifications/test`; deep-link routing `useNotificationDeepLinks` (tap → `router.push(data.link)`, running + cold-start); offline `OfflineBanner` over cached data on NETWORK error (consumer Home + Drops). **Proven:** gate 7/7; `scripts/smoke/slice16-push-smoke.mjs` 4/4 (OAuth mint + FCM v1 authorized project `gozaika`; invalid token → 400, not 401/403). **✅ DEVICE DELIVERY PROVEN 2026-06-29:** owner supplied `google-services.json` for both apps (gitignored; wired via `android.googleServicesFile`). Consumer app rebuilt with embedded FCM config → real device token obtained → live FCM v1 push via the service-account (`scripts/smoke/slice16-send-to-token.mjs`, 200 + message name) → notification rendered on the Pixel 7a (`.codex-artifacts/mobile-ux-uplift/android-preview-build/consumer-push-delivered.png`). **Deployed-BFF note:** set `FCM_SERVICE_ACCOUNT_JSON`/`_PATH` on the customer + partner BFFs for in-app `/notifications/test` + event-driven sends. Biometric app-lock + App Links association files are deferred follow-ups (not blocking).
