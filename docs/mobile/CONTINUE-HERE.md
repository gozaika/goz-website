...
- Next uplift slice: **D1 Demo/presales readiness** (seed rich demo scenarios + assets so Maestro flows are sales/investor-ready, no fake claims). See the slice table in `project docs/gozaika-mobile-ux-uplift-analysis-v1.md` §A5. **D1 is investor/partner/employee-facing** — make web + mobile look alive: fleshed-out profiles for every seed customer + restaurant, plus aesthetically attractive store/app imagery.
