...
- **D1 Demo/presales readiness — IN PROGRESS** on branch `codex/mobile-ux-uplift/d1-demo-presales`. Investor/partner/employee-facing. **Key finding:** the demo seed (`supabase/seed_demo/`) is already rich (8 consumers, 5 restaurants with vivid headline+story copy, 17 drops, 28 orders, reviews, payments, finance, passport, subscriptions) — so D1 is an **imagery + store-asset** slice, not a data-enrichment one. Imagery = **original SVG/vector art by the agent** (owner-approved; stock photos can't be pulled into the repo here, and reusing real restaurants' photos is a licensing problem).
  ...
  - **D1b done + QA'd:** cuisine-keyword logic extracted to `@gozaika/utils#cuisineCoverKey` (single source of truth; mobile `coverFor` now uses it). Cover SVGs copied to `apps/consumer-web/public/art/`. Web wired: restaurant-detail hero (cuisine art behind a forest `/92–/96` overlay, white text stays AA), shared `@gozaika/ui` `DropCard` cover banner, and the inline "Closing soon" cards. Typecheck (ui/consumer-web/utils/mobile) + mobile gate 7/7. **Web visual QA captured 2026-06-27** against live cloud demo data: hero contrast AA-readable on all 3 cuisine extremes, DropCard banners cuisine-accurate + cleanly cropped (clipped to `rounded-lg`). No art fixes needed. Evidence (gitignored): `.codex-artifacts/mobile-ux-uplift/web-preview/d1b-*.png`. To run the preview yourself: `.claude/launch.json` defines a `consumer-web` config; Next loads `.env.local` from the **app dir**, so `cp .env.local apps/consumer-web/.env.local` (gitignored) before `next dev`.
  - **Demo-data mojibake FIXED (2026-06-27, owner-directed, found during D1b QA):** cloud demo copy rendered literal `???` where em-dashes belong (`restaurant_public_profile.headline`/`story_markdown`, `catalog_bag_template_revision.short_description`/`included_item_hint_text`, `drop_drop.drop_title`) — seed source is correct UTF-8 `—`; the cloud DB mangled em-dashes at apply time. Audited (all em-dashes, no other multibyte chars), verified each `???→—` against the seed verbatim (27/27), applied via service-role PATCH; 0 remaining across 11 tables. Idempotent guarded repair committed at `supabase/seed_demo/demo_fix_mojibake.sql` — **add it to the reseed recipe** (run after parts 1–2, or any time `???` reappears).
  - **D1c done + QA'd:** drop-type / blind-bag art variants. Shared `@gozaika/utils#dropCoverKey(drop)` returns the cuisine-agnostic `"mystery"` cover for `BLIND_ADVENTURE` (blind bags no longer reveal cuisine) else the cuisine cover; `dropTypeRibbon(code)` → "Chef's Special"/"Spotlight". New `cover-mystery` art (cloche + gold "?") via `scripts/demo-art/build-art.mjs`. Web `DropCard` + "Closing soon" cards use `dropCoverKey` + a gold (charcoal-on-gold, AA) ribbon; mobile `mediaFallbacks.coverForDrop` picks the mystery PNG, wired into the 3 drop fallbacks. 6 utils tests; gate 7/7. Web QA confirmed all 4 drop types on `/drops` (evidence `.codex-artifacts/mobile-ux-uplift/web-preview/d1c-*.png`); on-device Android capture deferred to the slice-D manual inspection.
  - **D1d done — reconciliation (then superseded by D1f):** `docs/mobile/d1d-store-asset-reconciliation.md` reconciled with `.codex-artifacts/gozaika-polish-v2/`. It originally deferred card *production* to Codex's lane — but **Codex has since stopped and this agent owns the whole repo**, so D1f below actually produces the cards.
  - **D1e done + run on device:** `docs/mobile/d1e-demo-capture-manifest.md` (demo capture index) + additive `apps/consumer-mobile/.maestro/demo-discovery-vibrant.yaml`. Flow executed green on the Pixel 7a (3 stills to `.codex-artifacts/mobile-ux-uplift/demo-captures/`); on a release build, foreground the app first (the flow is devclient-safe / no `launchApp`).
  - **D1f done — store cards PRODUCED (owner-directed; Codex stopped):** real native Pixel 7a captures of the live cloud demo (windows rolled forward via the sanctioned `demo_prepare`; consumer rebuilt with D1a/D1c art) → **7 customer + 7 partner** store cards. Pipeline `scripts/store-cards/build-cards.mjs` (+ `cards.config.mjs`), deliverables in the **tracked** `store-assets/` (so they're pushed, not lost). Built to the polish-v2 design-system lock (1080×1920, palette, type scale, safe zones, allowed badges, real-UI-only). Partner captured signed-in as OWNER. Caveats (demo `GZ-SEED` IDs, partner revenue/AOV needs finance approval, Zayka/goZaika passport naming, iOS 1290×2796 derivative, final typography) in `store-assets/README.md`.
  - **✅ D1 COMPLETE (D1a–D1f), 2026-06-27 — awaiting owner manual inspection, then fast-forward `codex/mobile-ux-uplift/d1-demo-presales` → `main`.** Branch tip carries D1a–D1f; gate 7/7; pushed to `origin/<branch>` + (per owner) `origin/main`. Mojibake repair applied + scripted (`supabase/seed_demo/demo_fix_mojibake.sql`).
