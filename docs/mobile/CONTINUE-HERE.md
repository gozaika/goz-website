...
- **D1 Demo/presales readiness — IN PROGRESS** on branch `codex/mobile-ux-uplift/d1-demo-presales`. Investor/partner/employee-facing. **Key finding:** the demo seed (`supabase/seed_demo/`) is already rich (8 consumers, 5 restaurants with vivid headline+story copy, 17 drops, 28 orders, reviews, payments, finance, passport, subscriptions) — so D1 is an **imagery + store-asset** slice, not a data-enrichment one. Imagery = **original SVG/vector art by the agent** (owner-approved; stock photos can't be pulled into the repo here, and reusing real restaurants' photos is a licensing problem).
  ...
  - **D1d done — reconciliation, NOT cards:** reconciled with `.codex-artifacts/gozaika-polish-v2/` (the authoritative, gated store-asset pipeline; polished creatives are Codex's lane). Producing store cards here would duplicate/conflict, so D1d = `docs/mobile/d1d-store-asset-reconciliation.md`: what D1 improved in the polish-v2 customer sources (CCard 1/2/3/7) + native recapture handoff. Partner sources unaffected (D1 art is consumer-only). No files written into the polish-v2 tree.
  - **D1e done:** `docs/mobile/d1e-demo-capture-manifest.md` (demo/presales capture index over the `apps/*/.maestro/` flows, with personas/preconditions/captures + the slice-D states each shows) + a new additive, robust `apps/consumer-mobile/.maestro/demo-discovery-vibrant.yaml` (Home rail + Drops grid; no sign-in; title-independent). Gate 7/7. Flow not executed here (no device) — run in the device inspection.
  - **✅ D1 COMPLETE (D1a–D1e), 2026-06-27 — awaiting owner manual inspection of all of slice D, then fast-forward `codex/mobile-ux-uplift/d1-demo-presales` → `main`.** Do NOT merge before the owner inspects. Branch tip carries D1a–D1e; gate 7/7. The cloud demo mojibake repair (owner-directed) is applied + scripted (`supabase/seed_demo/demo_fix_mojibake.sql`).
