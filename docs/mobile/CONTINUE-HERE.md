...
- Next uplift slice: **X1 A11y/motion/perf pass** (reduced-motion, Dynamic Type, contrast re-audit on the new overlays), then **D1 Demo/presales readiness**. See the slice table in `project docs/gozaika-mobile-ux-uplift-analysis-v1.md` §A5.
- **Uplift merge debt:** U1→R4 are all unmerged on their own chained branches; `main` still has the pre-uplift mobile screens. A consolidation/merge of the uplift chain into `main` is outstanding and should be planned before store-asset recapture.
