...
- **Phase 3 remaining:** RM-1 (mobile ROI parity — reconcile with web `loadRoiReport`; Pixel 7a `3A021JEHN02437` now connected + emulator), §19 (template archetype + allergen envelope — the one non-cosmetic change), Order Again in orders/counter. Warm-env mobile captures still open: §16 allergen gate (login as Rahul), CM-1, CM-2.
