...
- **TASK C (mobile on-device verify) — BLOCKED on a fresh native build.** Metro (watch) runs from real source; devices online (Pixel 7a `3A021JEHN02437` + emulator, `adb reverse tcp:8081`). Diagnosed: the installed `in.gozaika.customer` was a Jun-29 **release** APK with an embedded pre-Phase-2 JS bundle (does NOT load Metro → no Phase 2, no testIDs). The only prebuilt dev-client (`C:\tmp\gozaika-build/.../debug/app-debug.apk`, Jun 25) loads Metro JS but its native layer is too old → redboxes `Cannot find native module 'ExpoPushTokenManager'` (expo-notifications added after Jun 25). The build copy `C:\tmp\gozaika-build` is ALSO stale (no Phase 2 JS; node_modules missing expo-notifications). **To unblock:** sync the copy to `claude-feature-parity` → `npm install` → `npx expo run:android` from the short path (MAX_PATH) → current dev-client that loads Metro JS. Then verify + screenshot the 5 Phase 2 surfaces. Device restored to the working release APK for now. First mobile testID embedded: `screen:drops-list` on `(tabs)/drops/index` root (source only, unverified on-device). See `docs/screenshots/consumer-mobile/INDEX.md` build-blocker note.
- **NEXT (owner decision):** (a) do the mobile native rebuild to finish Task C, and/or (b) build the Playwright web-capture harness + capture the already-verified WEB Phase 2 surfaces (tractable now, no rebuild) → then Task D tests + Phase 3.
