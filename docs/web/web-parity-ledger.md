...
| `/portal/templates` | `/api/portal/templates*` | `templates` | Tokenize form + list | W5 | **Done** — template create/edit form + library list tokenized (saffron+charcoal publish, danger archive, forest success message); allergen/disclosure inputs on tokens. **Template primary image uploader added on edit path; image carries forward across new immutable revisions and can be overridden per drop.** Zero brand-hex. demo-OWNER browser-verified |
